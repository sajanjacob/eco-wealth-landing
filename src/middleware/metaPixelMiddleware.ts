...
import { sendViewContentEvent, sendInitiateCheckoutEvent } from '../presentation/utils/metaPixel';

export async function middleware(request: NextRequest) {
  console.log('Meta Pixel Middleware - Request path:', request.nextUrl.pathname);
  ...
  if (!clientIp || !userAgent) {
    console.log('Meta Pixel Middleware - Missing required headers, skipping events');
  ...
  try {
    ...
    if (request.nextUrl.pathname === '/register') {
      ...
      console.log('Meta Pixel Middleware - No matching event for path:', request.nextUrl.pathname);
    }
  } catch (error) {
